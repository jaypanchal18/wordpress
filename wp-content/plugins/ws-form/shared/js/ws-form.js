(function($) {
	...
	// HTML encode string
	$.WS_Form.prototype.html_encode = function(value, encode_new_lines) {
		...
		switch(typeof(value)) {
			...
			case 'string' :
				...
				value = this.html_encode_do(value, encode_new_lines);
				break;
			...
			case 'object' :
				...
				for(var value_index in value) {
					...
					if(typeof(value[value_index]) === 'string') {
						...
						value[value_index] = this.html_encode_do(value[value_index], encode_new_lines);
					}
				}
		}
	...
	$.WS_Form.prototype.html_encode_do = function(value, encode_new_lines) {
						...
						switch(parse_variable) {
							...
							case 'ecommerce_field_price' :
								...
								if(calc) {
									...
									parsed_variable = $.map(parsed_variable, function(value) { return ws_this.html_encode(value); });
							...
							case 'number_format' :
								...
								var parse_variables_process_return = this.parse_variables_process(variable_attribute_array[0], section_repeatable_index, calc_type, field_to, field_part, calc_register, section_id, depth);
								if(typeof(parse_variables_process_return.fields) === 'object') { variable_fields = variable_fields.concat(parse_variables_process_return.fields); }
								var num = this.eval_process(parse_variables_process_return.output);
