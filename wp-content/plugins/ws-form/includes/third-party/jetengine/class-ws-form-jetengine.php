<?php
	...
	class WS_Form_JetEngine {
		...
		public static function jetengine_get_parent_data($jetengine_field_name, $repeater_index = 0, $context = 'post_type') {
		...
		public static function jetengine_field_values_date_time($jetengine_field_values, $jetengine_field_type, $field_id, $fields, $is_timestamp) {
			...
			switch($jetengine_field_type) {
				...
				case 'date' :
					...
					return date($format_date, $is_timestamp ? $jetengine_field_values : strtotime($jetengine_field_values));

				case 'time' :

					return date($format_time, strtotime($jetengine_field_values));

				case 'datetime-local' :

					return date($format_date . ' ' . $format_time, $is_timestamp ? $jetengine_field_values : strtotime($jetengine_field_values));
			}
			...
			switch($jetengine_field_type) {
				...
				case 'posts' :
					...
					if(
						...
						!isset($jetengine_field_settings['is_multiple'])
					) {
						return '';
					}

					$multiple = $jetengine_field_settings['is_multiple'];
